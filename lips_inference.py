...
# Import model architecture from lips_segmentation.py
from lips_segmentation import UNet

def parse_args():
    parser = argparse.ArgumentParser(description='Lips Segmentation Inference')
    parser.add_argument('--image', type=str, required=True, help='Path to input image')
    parser.add_argument('--model', type=str, default='output_lips/lips_segmentation_model.pth', 
                        help='Path to trained model')
    parser.add_argument('--output', type=str, default='lips_segmentation_result.png', 
...
def overlay_mask(img, mask, alpha=0.4, color=(128, 0, 128)):  # Purple color for lips
...
def save_results(original_img, predicted_mask, output_path, overlay=False):
    ...
    if overlay:
        ...
        plt.title('Lips Segmentation Result')
        plt.axis('off')
    else:
        ...
        plt.title('Lips Segmentation Mask')
        plt.axis('off')
