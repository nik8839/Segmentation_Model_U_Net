...
# Dataset class to load images and lips masks
class LipsSegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Create empty binary mask for lips
        mask = np.zeros((IMG_SIZE, IMG_SIZE), dtype=np.uint8)
        
        # Find and merge upper lip, lower lip, and mouth masks
        for lip_type in ['u_lip', 'l_lip', 'mouth']:
            mask_path_pattern = os.path.join(MASK_ROOT, f"*/{img_id}_{lip_type}.png")
            lip_mask_paths = glob.glob(mask_path_pattern)
            
            if lip_mask_paths:
                lip_mask_path = lip_mask_paths[0]
                lip_mask = np.array(Image.open(lip_mask_path).convert('L'))
                lip_mask = cv2.resize(lip_mask, (IMG_SIZE, IMG_SIZE))
                lip_mask = (lip_mask > 0).astype(np.uint8)
                mask = np.maximum(mask, lip_mask)
...
def get_valid_image_ids():
    ...
    # Filter to only include images that have lip masks
    valid_ids = []
    for img_id in tqdm(image_ids, desc="Checking for lip masks"):
        u_lip_exists = len(glob.glob(os.path.join(MASK_ROOT, f"*/{img_id}_u_lip.png"))) > 0
        l_lip_exists = len(glob.glob(os.path.join(MASK_ROOT, f"*/{img_id}_l_lip.png"))) > 0
        mouth_exists = len(glob.glob(os.path.join(MASK_ROOT, f"*/{img_id}_mouth.png"))) > 0
        
        if u_lip_exists or l_lip_exists or mouth_exists:
    ...
    print(f"Found {len(valid_ids)} images with lip masks (limited to {DATASET_LIMIT})")
    return valid_ids
...
def main():
    ...
    # Create datasets
    train_dataset = LipsSegmentationDataset(train_ids, transform=transform)
    val_dataset = LipsSegmentationDataset(val_ids, transform=transform)
    ...
    # Save final model
    torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, 'lips_segmentation_model.pth'))
    ...
    print("Training complete! Model saved to:", os.path.join(OUTPUT_DIR, 'lips_segmentation_model.pth'))
