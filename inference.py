...
# Import model architecture from eye_segmentation.py
from eye_segmentation import UNet

def parse_args():
    parser = argparse.ArgumentParser(description='Eye Segmentation Inference')
    parser.add_argument('--image', type=str, required=True, help='Path to input image')
    parser.add_argument('--model', type=str, default='output/eye_segmentation_model.pth', 
                        help='Path to trained model')
    parser.add_argument('--output', type=str, default='eye_segmentation_result.png', 
...
def overlay_mask(img, mask, alpha=0.4, color=(0, 0, 255)):
...
def save_results(original_img, predicted_mask, output_path, overlay=False):
    ...
    if overlay:
        ...
        plt.title('Eye Segmentation Result')
        plt.axis('off')
    else:
        ...
        plt.title('Eye Segmentation Mask')
        plt.axis('off')
