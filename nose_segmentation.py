...
# Dataset class to load images and nose masks
class NoseSegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Create empty binary mask for nose
        mask = np.zeros((IMG_SIZE, IMG_SIZE), dtype=np.uint8)
        
        # Find nose mask
        mask_path_pattern = os.path.join(MASK_ROOT, f"*/{img_id}_nose.png")
        nose_mask_paths = glob.glob(mask_path_pattern)
        
        if nose_mask_paths:
            nose_mask_path = nose_mask_paths[0]
            nose_mask = np.array(Image.open(nose_mask_path).convert('L'))
            nose_mask = cv2.resize(nose_mask, (IMG_SIZE, IMG_SIZE))
            nose_mask = (nose_mask > 0).astype(np.uint8)
            mask = np.maximum(mask, nose_mask)
...
def get_valid_image_ids():
    ...
    # Filter to only include images that have nose masks
    valid_ids = []
    for img_id in tqdm(image_ids, desc="Checking for nose masks"):
        nose_exists = len(glob.glob(os.path.join(MASK_ROOT, f"*/{img_id}_nose.png"))) > 0
    ...
    print(f"Found {len(valid_ids)} images with nose masks (limited to {DATASET_LIMIT})")
    return valid_ids
...
def main():
    ...
    # Create datasets
    train_dataset = NoseSegmentationDataset(train_ids, transform=transform)
    val_dataset = NoseSegmentationDataset(val_ids, transform=transform)
    ...
    # Save final model
    torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, 'nose_segmentation_model.pth'))
    ...
    print("Training complete! Model saved to:", os.path.join(OUTPUT_DIR, 'nose_segmentation_model.pth'))
